...
export const triggerWebsiteCrawl = asyncHandler(async (req: Request, res: Response) => {
  const organizationId = req.user?.organization?.toString();
  ...
  if (!sourceUrl) {
    throw new AppError("Source URL is required to start crawling.", 400);
  }

  const workspace = await workspaceRepository.findByIdToUpdate(workspaceId as string);
  ...
  if (!workspace.isActive) {
    throw new AppError("Cannot crawl websites for an inactive workspace.", 400);
  }

  if (workspace.sourceUrl !== sourceUrl) {
    workspace.sourceUrl = sourceUrl;
    await workspace.save();
  }

  const job = await crawlerService.startWorkspaceCrawl(
  ...
  res.status(202).json({
    status: "success",
    message: "Website crawling initiated in the background. Pages will appear as they are processed.",
    data: {
      jobId: job._id
    }
  });
